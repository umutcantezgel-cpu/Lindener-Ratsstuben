...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const article = getRegionalArticle('ausflug', params.slug);
  ...
  return {
    ...
    alternates: {
      canonical: `https://lindener-ratsstuben.de/${params.locale}/ausflug/${params.slug}`
    }
  };
}

export async function generateStaticParams() {
  const slugs = getRegionalSlugsByCategory('ausflug');
...
export default function AusflugPage({ params }: PageProps) {
  const article = getRegionalArticle('ausflug', params.slug);
